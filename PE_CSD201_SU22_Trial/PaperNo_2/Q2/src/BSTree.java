...
class BSTree
  ...
  void breadthfirstsearch(Node p){
       if(p==null) return;
     Queue q = new Queue();
             if(p.left!= null && p.right!= null && p.info.price<7 ) {q.enqueue(p);}
...
//===========================================================================
 ...
 if(p.info.price == x.price)return ;
   ...
   void insert(String xOwner, int xPrice)
     ...
     if(cu.info == x) return ;
     father = cu ;
     if(cu.info.price<x.price) cu = cu.right;
     else if(cu.info.price>x.price) cu = cu.left;
     }
     assert(father != null);
     if(father.info.price>x.price) father.left = node;
